...
static void button_power_save_resume_deferred(void* arg, uint32_t value)
{
    (void)value;
...
static void IRAM_ATTR button_power_save_isr_handler(void* arg)
{
    ...
    gpio_intr_disable((gpio_num_t)(uintptr_t)arg);
    (void)xTimerPendFunctionCallFromISR(button_power_save_resume_deferred,
                                        arg,
                                        0,
